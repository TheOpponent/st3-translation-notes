...
class MainFrame(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        if self.filename != "":
            if os.stat(self.filename).st_size == 1178944:
                ...
                wx.MessageBox(
                    "Invalid SKFONT.CG file.", "Error", style=wx.OK | wx.ICON_ERROR
                )
                exit()
            ...
            for i in range(0, self.TILE_MAX):
                tile = FontTile(
                    self.file_data[
                        i * self.tile_data_length : i * self.tile_data_length
        ...
        self.pg.Append(
            wxpg.IntProperty("Tile Offset", "TileOffset", value=self.tile_offset)
        )
        ...
        self.pg.Append(
            wxpg.BoolProperty(
                "Invert (Preview Only)", "Invert", value=self.invert_image
            )
        )
        ...
        self.pg.Append(
            wxpg.IntProperty("Selected Tile", "SelectedTile", value=self.selected_tile)
        )
        ...
        self.pg.Append(
            wxpg.StringProperty(
                ...
                "SelectedTileLocation",
                value=str(hex(self.selected_tile)),
            )
        )
        self.pg.Append(
            wxpg.StringProperty(
        ...
        self.tilegrid_panel = GridBitmap(
            self, self.tile_grid, self.tile_rows, self.tile_cols
        )
        ...
        self.scrollbar.SetScrollbar(
            0, self.tile_rows, self.TILE_MAX // self.tile_cols, self.tile_rows
        )
    ...
    def save_skfont(self):
        ...
        try:
            ...
            wx.MessageBox(
                f"{self.filename} saved successfully.", "Save Complete", wx.OK
            )
            self.modified = False
    ...
    def on_replace_tiles(self, event):
        ...
        image_tiles = []
        new_tiles = []
        image_filename = wx.FileSelector(
            "Open Image", wildcard="*.png", flags=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST
        )
        if image_filename != "":
                ...
                for tile in image_tiles:
                    ...
                    for y in range(0, self.tile_size, 2):
                        ...
                        for x in range(0, self.tile_size):
                            ...
                            lo_pixel = (
                                converted_tile[x + ((y + 1) * self.tile_size)] & 0xF0
                            )
            ...
            with ReplaceTilesDialog(
                self, len(new_tiles), self.selected_tile, new_tiles_bitmap
            ) as dialog:
        ...
        with ExportTilesDialog(self, self.selected_tile) as dialog:
            if dialog.ShowModal() == wx.ID_OK:
                ...
                output_image = Image.new(
                    "RGB", (self.tile_size * tile_count, self.tile_size)
                )
                for i in range(0, tile_count):
                    output_image.paste(
                        self.tiles[self.selected_tile + i].image,
                        (self.tile_size * i, 0),
                    )
                save_filename = wx.FileSelector(
                    "Save PNG Image",
                    wildcard="*.png",
    ...
    def on_property_change(self, event):
        ...
        if prop:
            ...
            self.scrollbar.SetScrollbar(
                self.tile_offset // self.tile_cols,
                self.tile_rows,
                self.TILE_MAX // self.tile_cols + 1,
                self.tile_rows,
            )
            ...
            self.pg.SetPropertyValueString(
                "SelectedTileLocation",
                str(hex(self.selected_tile * self.tile_data_length)),
            )
            self.pg.SetPropertyValueString(
                "RAMLocation",
                str(hex(self.selected_tile * self.tile_data_length + 0x8CCD7EC0))[2:],
            )
    ...
    def on_close(self, event):
        if event.CanVeto() and self.modified:
            confirm = wx.MessageBox(
                "Save changes to SKFONT.CG before closing?",
                "Confirm Exit",
...
class ReplaceTilesDialog(wx.Dialog):
    ...
    def __init__(
        ...
        label = wx.StaticText(
            main_panel,
            label=f"Replacing {tile_count} tile(s) at tile number {selected_tile}.",
        )
...
class AboutDialog(wx.Dialog):
    def __init__(self, parent, version):
        ...
        text = wx.StaticText(
            panel,
            label=f"Mr. SKFONT version {version} by The Opponent\nPart of the Sakura Taisen 3 translation notes repository.\n\nThis program is in the public domain (Unlicense).",
            style=wx.LEFT,
        )
        link = hl.HyperLinkCtrl(
            panel,
            -1,
            "https://github.com/TheOpponent/st3-translation-notes",
            URL="https://github.com/TheOpponent/st3-translation-notes",
            style=wx.TE_LEFT,
        )
...
class GridBitmap(wx.Panel):
    ...
    def on_paint(self, event):
        ...
        dc.DrawLine(
            self.selected_tile[0] * self.cell_width - 1,
            self.selected_tile[1] * self.cell_height,
            (self.selected_tile[0] + 1) * self.cell_width + 1,
            self.selected_tile[1] * self.cell_height,
        )
        dc.DrawLine(
            self.selected_tile[0] * self.cell_width,
            self.selected_tile[1] * self.cell_height - 1,
            self.selected_tile[0] * self.cell_width,
            (self.selected_tile[1] + 1) * self.cell_height + 1,
        )
        dc.DrawLine(
            self.selected_tile[0] * self.cell_width,
            (self.selected_tile[1] + 1) * self.cell_height + 1,
        ...
        dc.DrawLine(
            ...
            self.selected_tile[1] * self.cell_height,
            (self.selected_tile[0] + 1) * self.cell_width + 1,
            (self.selected_tile[1] + 1) * self.cell_height,
        )
