...
def main():
    ...
    for translate_file in os.listdir(translate_path):
        ...
        if os.path.splitext(translate_file)[1] == ".csv" and os.path.isfile(
            lip_source := os.path.join(lip_path, translate_base_name)
        ):
                    ...
                    for i in csv_file:
                        ...
                        if re.fullmatch(
                            ...
                            line_encoded, warning = ascii_to_sjis(
                                i[2], line_id=i[1], filename=translate_file
                            )
                            warnings += warning
                            ...
                            new_cmd_sequence = [
                                new_cmd_sequence[i : i + DELAY]
                                for i in range(0, len(new_cmd_sequence), DELAY)
                            ]
                            ...
                            if len(new_cmd_sequence) < len(i[2]):
                                if len(new_cmd_sequence[-1]) < DELAY:
                                    new_cmd_sequence[-1] += last_char * (
                                        DELAY - len(new_cmd_sequence[-1])
                                    )
            ...
            with open(
                os.path.join(output_path, translate_base_name), "wb"
            ) as output_file:
                output_file.write(output_binary)
                print(
                    f"{translate_base_name}: {len(output_binary)} ({hex(len(output_binary))}) bytes written."
                )
