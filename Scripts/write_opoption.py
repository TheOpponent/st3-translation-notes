    ...
    for filename, offset_table, other_bytes, index_count in op_files:
        try:
            with open(os.path.join(source_path, filename), "rb") as bin_file:
                try:
                    with open(
                        os.path.join(translate_path, filename + ".csv"),
                        encoding="utf-8",
                    ) as file:
                        ...
                        # Seek to first index and offset.
                    ...
                    recalc_data = {
                        k: v for (k, v) in zip(original_offsets, new_offsets)
                    }
                    ...
                    with open(os.path.join(output_path, filename), "wb") as output_file:
                        ...
                        print(
                            f"{filename}: {len(output_binary)} ({hex(len(output_binary))}) bytes written."
                        )
